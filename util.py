...
def get_attack(node, unsorted=True, branch_attack = branch_attack):   # 选取进攻的点位
    ...
    if unsorted is True:
        ...
        for state in successor:
            newnode = Node(next_cd=state, who=nextwho, boardNow=node.board)
            newnode.evaluate()
            a = newnode.value
...
def get_successor(node, unsorted=True):
    ...
    successor = []
    t = 0
    if Free(pp.width // 2, pp.height // 2, board):
    ...
    if unsorted is True:
        succ_node = []
        for s in successor:
            succ_node.append(Node(next_cd=s, who=3 - node.who, boardNow=node.board))
