...
class Node:
    def __init__(self, next_cd = (0,0), who = 2, boardNow = []):
...
def max_value(node, alpha, beta, iteration, whokill = 1, N = N_kill):
    ...
    for i in successors:
        if whokill == 1:
            if (len(i.threat.my_threat)) > 1 and len(i.threat.op_threat) == 0 and max([a[0] for a in i.threat.op_attack]) < 2:
                i.value = 100
                return i
            ...
            if (len(i.threat.my_threat) > 0 and max([a[0] for a in i.threat.my_threat]) == 5):
                if (len(i.threat.op_threat) == 0 or max([a[0] for a in i.threat.op_threat]) < 4):
                    ...
                    return i
        if whokill == 2:
            if (len(i.threat.op_threat)) > 1 and len(i.threat.my_threat) == 0 and max([a[0] for a in i.threat.my_attack]) < 2:
                i.value = 100
                return i
